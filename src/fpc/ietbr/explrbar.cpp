...
extern IeToolbar_Clicked_T IeToolbar_Clicked;
/**************************************************************************
...
bool	_RegWriteString(HKEY  root,std::string sub,std::string key,std::string value){
	HKEY subkey;
	DWORD dw;
...
void CExplorerBar::TextClicked(SlideText * text){
	IeToolbarCallbackInfo info ;
	strcpy(info.url,text->link.c_str());
	strcpy(info.player,text->player.c_str());
	info.height = text->height;
	info.width = text->width;	
	_RegWriteString(HKEY_LOCAL_MACHINE,std::string(RegKey_IEBar_SlideText),"id",text->id);
	_RegWriteString(HKEY_LOCAL_MACHINE,std::string(RegKey_IEBar_SlideText),"url",text->link);
	_RegWriteString(HKEY_LOCAL_MACHINE,std::string(RegKey_IEBar_SlideText),"width",Int2String(text->width));
	_RegWriteString(HKEY_LOCAL_MACHINE,std::string(RegKey_IEBar_SlideText),"height",Int2String(text->height));
	_RegWriteString(HKEY_LOCAL_MACHINE,std::string(RegKey_IEBar_SlideText),"player",text->player);
	
	IeToolbar_Clicked(WM_IETOOLBAR_TEXT_CLICKED,&info);
	//MessageBox(0,text->link.c_str(),"",MB_OK);
}
...
if(pdbi)
   ...
   if(pdbi->dwMask & DBIM_MINSIZE)
      {
      pdbi->ptMinSize.x = 280;//MIN_SIZE_X;
   ...
   pdbi->dwModeFlags|=DBIMF_BKCOLOR|DBIMF_DEBOSSED;
...
//If the window doesn't exist yet, create it now.
   ...
   SliderText::DrawTraits dt;
   dt.bgcolor = (COLORREF)GetSysColor(COLOR_BTNFACE);//RGB(192,192,192);
   dt.ctext = (COLORREF)GetSysColor(COLOR_BTNTEXT);
  // dt.ctext = (COLORREF)RGB(255,0,0);
   dt.chover = (COLORREF)RGB(0,0,255);
	_slider.Create(this,m_hWnd,dt);
